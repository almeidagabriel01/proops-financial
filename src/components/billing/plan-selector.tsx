...
export function PlanSelector({ compact = false }: PlanSelectorProps) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-3">
      ...
      {/* Pro — sem trial */}
      <div className="flex flex-col rounded-2xl border border-border bg-card p-6">
        <p className="mb-1 text-xs font-bold uppercase tracking-wider text-muted-foreground">Pro</p>
        <div className="mb-1 flex items-baseline gap-0.5">
          <span className="text-3xl font-bold tracking-tight text-foreground">R$ 49</span>
          <span className="text-lg font-bold text-foreground">,90</span>
          <span className="ml-1 text-sm text-muted-foreground">/mês</span>
        </div>
        <p className="mb-4 text-sm text-muted-foreground">Sem trial. Assine e use agora.</p>
        <Button
          variant="outline"
          ...
          className="mt-auto"
        >
          {loading === 'pro_monthly-false' ? 'Aguarde...' : 'Assinar Pro agora'}
        </Button>
      </div>
    </div>
  );
